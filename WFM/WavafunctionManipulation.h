...
#define Matrix_Printf(M){}{ \
		int i,j;\
		for(i=0;i<M.row_size;i++)\
		{ for(j=0;j<M.col_size;j++)\
		{ printf("%lf",Matrix(M,j,i)), printf("%c",' ');\
		}printf("%c",'\n');}\
		}
#define Def_E(i_,j_,t_,Qx,a,b,Stack,StackValue,Return_Value){\
double p = a+b;\
double q = a*b/p;\
int StackSize = 0;\
...
{\
    ...
    StackValueSize = StackValueSize -1;\
    int t = Stack[StackSize+2];\
    int j = Stack[StackSize+1];\
    int i = Stack[StackSize];\
    if(t<0||t>(i+j))\
      continue;\
    else if(i==0&&j==0&&t==0)\
       {\
         Return_Value = Return_Value+StackValue[StackValueSize];\
         continue;\
       }\
    else if(j ==0)\
      {\
          StackValue[StackValueSize+1] = StackValue[StackValueSize]*(q*Qx/a);\
          StackValue[StackValueSize+2] = StackValue[StackValueSize]*(t+1.0);\
          StackValue[StackValueSize] = StackValue[StackValueSize]*(1/(2*p));\
          StackValueSize = StackValueSize+3;\
          Stack[StackSize+2] = t-1;\
          Stack[StackSize+1] = j;\
          Stack[StackSize] = i-1;\
          StackSize = StackSize+3;\
          Stack[StackSize+2] = t;\
     ...
     {\
          StackValue[StackValueSize+1] = StackValue[StackValueSize]*(q*Qx/b);\
          StackValue[StackValueSize+2] = StackValue[StackValueSize]*(t+1.0);\
          StackValue[StackValueSize] = StackValue[StackValueSize]*(1/(2*p));\
          StackValueSize = StackValueSize+3;\
          Stack[StackSize+2] = t-1;\
          Stack[StackSize+1] = j-1;\
          Stack[StackSize] = i;\
          StackSize = StackSize+3;\
          Stack[StackSize+2] = t;\
          Stack[StackSize+1] = j-1;\
     ...
     Return_Value = Return_Value*exp(-q*Qx*Qx);\
}

#define Def_R(t_,u_,v_,n_,p,PCx,PCy,PCz,RPC,Stack,StackValue,Return_Value)\
{\
  double T = p*RPC*RPC;\
  Return_Value =0;\
  int StackSize = 0;\
  ...
  {\
    StackSize = StackSize-4;\
    StackValueSize = StackValueSize -1;\
    int t = Stack[StackSize];\
    int u = Stack[StackSize+1];\
    int v = Stack[StackSize+2];\
    int n = Stack[StackSize+3];\
    if(t==0&&u==0&&v==0)\
      {\
          Return_Value = Return_Value + pow(-2*p*1.0,n*1.0)*boys(n*1.0,T*1.0)*StackValue[StackValueSize];\
          continue;\
      }\
    else if (t==0&&u==0)\
    {\
       ...
       {\
            Stack[StackSize]=t;\
            Stack[StackSize+1]=u;\
            Stack[StackSize+2]=v-2;\
            Stack[StackSize+3]=n+1;\
            StackSize = StackSize +4;\
            StackValue[StackValueSize] = StackValue[StackValueSize]*(v-1);\
            StackValueSize++;\
       }\
            Stack[StackSize]=t;\
            Stack[StackSize+1]=u;\
            Stack[StackSize+2]=v-1;\
            Stack[StackSize+3]=n+1;\
            StackSize = StackSize +4;\
            StackValue[StackValueSize] = StackValue[StackValueSize]*PCz;\
            StackValueSize++;\
    }\
    else if(t==0)\
    {\
      ...
      {\
            Stack[StackSize]=t;\
            Stack[StackSize+1]=u-2;\
            Stack[StackSize+2]=v;\
            Stack[StackSize+3]=n+1;\
            StackSize = StackSize +4;\
            StackValue[StackValueSize] = StackValue[StackValueSize]*(u-1);\
            StackValueSize++;\
      }\
            Stack[StackSize]=t;\
            Stack[StackSize+1]=u-1;\
            Stack[StackSize+2]=v;\
            Stack[StackSize+3]=n+1;\
            StackSize = StackSize +4;\
            StackValue[StackValueSize] = StackValue[StackValueSize]*PCy;\
            StackValueSize++;\
    }else\
    {\
        if(t>1)\
        {\
            ...
            Stack[StackSize+3]=n+1;\
            StackSize = StackSize +4;\
            StackValue[StackValueSize] = StackValue[StackValueSize]*(t-1);\
            StackValueSize++;\
        }\
            Stack[StackSize]=t-1;\
            Stack[StackSize+1]=u;\
            ...
            Stack[StackSize+3]=n+1;\
            StackSize = StackSize +4;\
            StackValue[StackValueSize] = StackValue[StackValueSize]*PCx;\
            StackValueSize++;\
     }\
  }\
}
