...
double coulomb_repulsion(double xa, double ya, double za, double norma,
  ...
  Bx = B_array(la,lb,lc,ld,xp,xa,xb,xq,xc,xd,gamma1,gamma2,delta,X_);
  By = B_array(ma,mb,mc,md,yp,ya,yb,yq,yc,yd,gamma1,gamma2,delta,Y_);
  Bz = B_array(na,nb,nc,nd,zp,za,zb,zq,zc,zd,gamma1,gamma2,delta,Z_);
...
double *B_array(int l1, int l2, int l3, int l4, double p, double a,
		...
		double g1, double g2, double delta,int Type){
  int Imax,i1,i2,r1,r2,u,I,i;
  double *B;
  Imax = l1+l2+l3+l4+1;
  B = (double *)(Buffer+Type*ITMAX);
...
double nuclear_attraction(double x1, double y1, double z1, double norm1,
  ...
  Ax = A_array(l1,l2,xp-x1,xp-x2,xp-x3,gamma,X_);
  Ay = A_array(m1,m2,yp-y1,yp-y2,yp-y3,gamma,Y_);
  Az = A_array(n1,n2,zp-z1,zp-z2,zp-z3,gamma,Z_);
